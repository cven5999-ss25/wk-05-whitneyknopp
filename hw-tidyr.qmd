...
1.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The `tidyverse` package is a collection of packages in `R` that improve capacity to handle and tidy data effectively. It includes multiple packages, making it easier than calling many libraries.
...
2.  What's the [`here` Package](https://here.r-lib.org/articles/here.html)? Why is useful when working with quarto or R Markdown files?
    ...
    The `here` package is designed to help the navigation and referencing of file paths. It is helpful when working with quart or R Markdown files because they utilize file paths relative to the top of the file directory and makes them safe to pass through other functions.
...
```{r vector_coresion}
# create frequency table for income categories to confirm spelling 
waste_data |> 
  count(income_cat)
...
```{r wide_to_long}
# convert the data from a wide format to long format
...
```{r summarize}
# summarize the data, grouped by income and waste category
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    I would not publish a table in this format in a report.
...
3.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    I would put this table in a report, but to make it look even better, I will use `knitr::kable()` to clean up the table.

4.  Render and fix any errors

```{r long_to_wide}
# convert the long data means to a wide format
...
```{r plotting}
# plot the data to match the figure attached
ggplot(data = waste_data_long_mean, 
  ...
  labs(title = "Waste Composition",
         ...
         x = "mean (percent)",
         y = "waste category",
         caption = "Data from: https://datacatalog.woldbank.org/search/dataset/0039597",
